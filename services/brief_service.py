...
class BriefService:
    ...
    def generate(self, hours: int = 24) -> DailyBrief:
        ...
        if self.llm_client is None:
            raise RuntimeError(
                "LLM 客户端未初始化，可能原因：API Key 未配置、"
                "LLM provider 未知，或 API 地址无法访问。"
                "请检查 .env 中的 llm_provider、llm_api_key、llm_base_url 配置。"
            )
        ...
        try:
            ...
            if "Connection" in err_msg or "connect" in err_msg.lower():
                raise RuntimeError(f"无法连接到 LLM API：{err_msg}") from e
            if "401" in err_msg or "403" in err_msg or "Unauthorized" in err_msg:
                raise RuntimeError(f"LLM API 认证失败，请检查 API Key：{err_msg}") from e
            if "429" in err_msg or "Rate" in err_msg or "rate" in err_msg:
                raise RuntimeError(f"LLM API 请求被限流，请稍后重试：{err_msg}") from e
            if "timeout" in err_msg.lower() or "timed out" in err_msg.lower():
                raise RuntimeError(f"LLM API 请求超时：{err_msg}") from e
