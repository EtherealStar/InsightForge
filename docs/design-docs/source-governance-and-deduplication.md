...
- 全局 Intel Fact、多证据绑定、Evidence Role 和 Verification Status。
...
| D | 已审核确认缺乏可靠溯源、低质量聚合或低质量转载的来源 | 隔离；正文最多保留 7 天 |
...
去重位于 Markdown 转换和页面质量过滤之后、Document Version 入库和任何分块/Embedding/LLM 调用之前：
...
阈值不是永久常量。正式删除重复正文前必须先影子运行，以人工标注集校准；自动合并精确率目标不低于 98%。
...
6. 异步删除旧 Qdrant points 和非审计正文；失败时继续服务旧版本。
...
## 8. Evidence Role 与事实验证

### 8.1 全局 Intel Fact

Intel Fact 表示全局规范化事实，不再强制从属于单个 `source_document_id`。多个独立文档通过 Evidence Reference 支持同一个 fact。

候选事实先使用规范化 `assertion_key` 召回：主体、fact 类型、谓词、对象和事件日期。文字不同的候选交由结构化抽取 LLM 判断：
...
模型必须返回原文引用、判断理由、模型和 Prompt 版本。规则冲突或输出不完整时降级为待复核，不自动合并事实。

### 8.2 Evidence Role 分类

Evidence Role 需要理解证据相对于 fact 的语义，采用“LLM 提议、确定性规则校验、冲突降级、人工覆盖”：

| 角色 | 含义 |
|---|---|
| `primary` | 对客观发布、价格、版本或官方行为的一手来源 |
| `independent` | 与事实主体无利益归属的独立报道或验证 |
| `interested_claim` | 来源主体对自身或竞品作出的利益相关声明 |
| `community_report` | 用户、论坛或个人经验报告 |
| `aggregator` | 聚合或转述内容，缺少独立采编 |
| `unknown` | 语义证据不足或模型与规则冲突 |

代码负责域名归属、source kind、竞品关系、重复簇关系等客观约束；LLM 负责判断原文是客观公告、自我评价、比较主张还是经验陈述。社区来源不能被升级为官方 primary，聚合站不能被标为独立原始调查。人工覆盖必须记录原因和审计信息。

### 8.3 Verification Status

`IntelFact.status` 继续表达编辑工作流：`draft`、`active`、`rejected`、`archived`。新增独立的 `verification_status`：
...
同一 Document Cluster 中的多个 Source Occurrence 只计为一个证据来源。单个 C 级来源不能得到 `corroborated`。自动验证只更新 `verification_status`，不自动把 `draft` 改为 `active`。

删除 `source_reliability`。系统保留 Source Tier 快照、Evidence Role、抽取结果、Verification Status 和解释原因，不计算统一权威总分。
...
该权重仅是检索先验，不等同于事实可信判断。事实是否可以用于报告结论仍由 Evidence Role、Verification Status 和报告质量门禁决定。
...
- `EvidenceVerificationService`：Evidence Role 校验、全局 fact 归并和 Verification Status 计算。

不要把上述职责继续堆入 `PipelineService` 或扩张现有 `DocumentStoreProtocol` 成为一个包含所有治理行为的宽接口。Pipeline 只编排阶段并消费明确的领域结果：`new_cluster`、`duplicate`、`review_required`、`canonical_promoted`、`quarantined` 或 `unchanged`。

## 11. Celery 任务

目标任务拆分：

- 单来源抓取任务：网络 I/O 和 Markdown 转换。
- 单文章准入/去重任务：来源门禁、指纹和短事务归簇。
- Document Version 构建任务：分块、Embedding、Qdrant、FTS 和事实抽取。
- 主来源晋升任务：构建新版本并原子切换。
- 重复簇对账任务：发现并合并异常漏网簇。
- 隔离正文清理任务：删除超过 7 天的 D 级正文，保留轻量审计元数据。
...
- `unverified`、`self_reported`、`corroborated`、`disputed` facts 分布。
- Evidence Role 规则冲突率和人工覆盖率。

正式启用自动正文删除前，影子运行必须满足：
...
1. **基础模型**：Source Profile、评级历史、Document Cluster、Source Occurrence、Document Version、Protocol 和 PostgreSQL Store。
2. **影子去重**：计算精确哈希、SimHash 和 shingle 相似度，只记录判定和样本。
3. **正式去重**：Redis 前置索引、PostgreSQL 分段锁、重复归簇、D 级 7 天清理。
4. **主来源晋升**：版本化构建、原子切换、失败回滚和后台对账。
5. **可信证据**：全局 Intel Fact、多 Evidence Reference、LLM Evidence Role、规则校验和 Verification Status。
6. **检索与治理**：C 级轻微降权、来源/重复/冲突治理界面、审计与指标。

每个阶段必须先完成存储与服务层测试，再接入 Pipeline。阶段 2 的影子数据和人工结论是阶段 3 阈值配置的前置条件。
