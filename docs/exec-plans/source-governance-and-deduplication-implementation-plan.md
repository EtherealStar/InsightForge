...
- [x] 启用 PostgreSQL 并发归簇、Redis 热点索引、正式去重、对账和隔离清理（事务内 band + shingle 复核已接入；Redis 全量重建任务已补齐）。
- [x] 实现 Canonical Article 选择、派生数据版本化构建和 active version 原子切换（chunk/point 身份已绑定 document_version_id，PostgreSQL 父块查询只返回 active version）。
- [x] (2026-07-12 17:00+08:00) Intel Fact 已切换为全局身份，并完成 Evidence Reference 的版本/occurrence 绑定、角色/立场和证据驱动验证状态；Evidence API 不再接受客户端伪造来源等级。
- [x] (2026-07-12 17:05+08:00) 接入治理来源 API 与 Vue 工作台，删除业务层 `source_reliability` 路径；重复候选操作、版本晋升和证据角色人工覆盖 API 仍待补齐。
- [ ] 完成全量测试、端到端演练、缓存重建演练、文档同步和生产启用验收（全量测试仍受仓库现有 evals 包缺失和 embedding mock 构造器不兼容影响）。
...
实施记录（2026-07-12 17:10+08:00）：新增 `008_versioned_evidence.sql`、`009_governance_audit.sql`、`010_occurrence_shingles.sql`；Evidence Reference 现在必须由 PostgreSQL 解析到 active Document Version 与 Source Occurrence，验证状态按独立簇、来源等级和反证重算。事务归簇在 advisory lock 内执行 band、汉明距离和 shingle 复核，并记录灰区候选；Pipeline 已创建/激活/失败 Document Version，Canonical 晋升触发版本构建。治理 API/工作台已支持来源列表、待审队列、修订历史和带 actor/reason 的评级。

2026-07-12 focused regression: 32 passed; full suite: 273 passed, 8 skipped, 25 failed. The 25 failures remain the pre-existing missing `evals` package (24) and embedding mock incompatibility (1). `pnpm --dir frontend build` passed; `compileall` and `git diff --check` passed. Docker Compose services are running (`postgres`, `redis`, `qdrant`), but `migrations/apply_migrations.py` could not be launched by the Windows command runner (permission denied), so SQL integration and end-to-end Redis rebuild remain unverified.
...
实施记录（2026-07-12 18:00+08:00）：新增 `DedupMaintenanceService` 与 `rebuild_dedup_cache_task`，按 PostgreSQL occurrence 分批清空并重建 Redis 热点索引；新增重复候选列表和 Canonical 来源晋升治理 API，晋升操作写入 actor/reason/before/after 审计。chunk/point/parent ID 现绑定 `document_version_id`，父块按 active version 过滤，避免 Canonical 晋升造成新旧派生数据混读。聚焦回归 16 passed，compileall 与 `git diff --check` 通过。

全量测试记录（2026-07-12）：275 passed、8 skipped、25 failed。失败均为工作树既有问题：24 项依赖当前缺失的 `evals` 包模块，1 项 embedding 测试 fake OpenAI 不接受现有 `http_client` 参数；本次治理改动聚焦回归未复现失败。
