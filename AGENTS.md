...
1. **Protocol 优先**：`core/protocols.py` 定义 7 个接口（含 `WebhookServiceProtocol`），`infrastructure/` 实现，`services/` 通过 Protocol 调用
2. **工厂模式**：`core/factory.py` 创建基础设施 + Service 层实例（12 个工厂函数），`ConfigManager` 单例缓存 + 热重载
...
| 文件 | 作用 |
|---|---|
| `core/protocols.py` | 7 个 Protocol 接口定义 |
| `core/factory.py` | 12 个工厂函数（基础设施 8 + Service 4） |
...
| `evals/eval_config.json` | 评判 LLM 配置（OpenAI 兼容） |
