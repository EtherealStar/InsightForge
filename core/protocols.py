...
class CollectionRunStoreProtocol(Protocol):
    ...
    def advance_task(self, task_id: str, status: str, error: dict[str, Any] | None = None) -> None: ...
...
class FetchArtifactStoreProtocol(Protocol):
    ...
    def list_expired_blob_paths(self, before: datetime, limit: int = 1000) -> list[str]: ...
    def latest_for_url(self, normalized_url: str) -> RawFetchArtifact | None: ...


@runtime_checkable
class FetchCandidateStoreProtocol(Protocol):
...
@runtime_checkable
class FetchBlobStoreProtocol(Protocol):
...
@runtime_checkable
class NormalizedDocumentStoreProtocol(Protocol):
...
@runtime_checkable
class SourceRateLimiterProtocol(Protocol):
